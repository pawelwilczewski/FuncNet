...
internal static class ZipExtensionsGenerator
{
	...
	private static MethodBuilder GenerateMethod(MethodGenerationParams p) =>
		new MethodBuilder($"public {(p.MethodType is MethodType.Extension ? "static" : "")} {"TResult".WrapInAsyncTaskIf(p.IsAsync(UnionMethodAsyncConfig.ReturnType))}"
				+ $" {p.MethodName}<TResult, {p.Ts().CommaSeparated()}>")
